...
var AdminAccount = mongoose.model('Administrator');
...
router.post('/register', function (req, res, next) {
    ...
    if (c==0 && SecuritiesAccount.find({userID : userIdno, isActivated : 1, type : 0}).count() > 0) {
        res.send("身份证号已被使用!");
    }

    if (c==1 && SecuritiesAccount.find({docID : docid, isActivated : 1, type : 1}).count() > 0) {
        res.send("法人编号已被使用!");
    }

    var secID = Math.ceil((1.0+Math.random())*1111111111);
    newAccount.saId = secID;
    ...
    newAccount.save( function (err) {
        ...
        res.status(200);
        //res.redirect('/SecAccSys/login');
        res.render('SecAccSys/pass', {stockId: secID});
    });
});
